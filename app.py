...
# TODO: key functions parsing functions to modularize
# find_heat_divs(round_soup)
# find_heat_id(heat_soup)
# find_heat_athlete_names_and_scores(heat_soup)
# find_heat_status(heat_soup)
# find_round_ids(event_results_soup)
# find_event_urls(mct_year_soup)
  ...
  def __call__(self, url):
    ...
    try:
      with urllib.request.urlopen(url) as html:
        app.logger.info('Request to {} successful'.format(url))
        self.last_call_time = time.time()
        return bs(html, 'lxml')
    except urllib.request.HTTPError as e:
      e('Unrecognized url {}'.format(url))
...
class Heat:
  ...
  def _update_status(self, div):
    ...
    if self.status == 'over':
      ...
      app.logger.info('\t\t{} complete'.format(self))
...
class Event:
  ...
  def _update_rounds(self):
    while True:
      if self._completed:
        app.logger.info('Event {} complete\n\n'.format(self))
        break
      elif (
...
class League:
  ...
  def create_event(self, event_name, draft_date=None):
    if event_name in [event.name for event in self.events]:
      app.logger.warning('Event {}-{} already exists'.format(
    ...
    for event in self.events_info.keys():
      if event != 'freshwater-pro': # lazily excluding for now
        try:
          ...
          app.logger.warning(
            'Event {}-{} not ready to begin'.format(event, self.year))
          continue
